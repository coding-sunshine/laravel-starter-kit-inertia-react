        ...
        return new Promise((resolve, reject) => {
            ...
            request.onupgradeneeded = (event) => {
                ...
                if (!db.objectStoreNames.contains('rakes')) {
                    const rakeStore = db.createObjectStore('rakes', {
                        keyPath: 'id',
                    });
                    rakeStore.createIndex('siding_id', 'siding_id', {
                        unique: false,
                    });
                    rakeStore.createIndex('rake_number', 'rake_number', {
                        unique: true,
                    });
                }
                ...
                if (!db.objectStoreNames.contains('vehicles')) {
                    ...
                    vehicleStore.createIndex(
                        'vehicle_number',
                        'vehicle_number',
                        { unique: true },
                    );
                    vehicleStore.createIndex('rfid_tag', 'rfid_tag', {
                        unique: false,
                    });
                }

                if (!db.objectStoreNames.contains('indents')) {
                    const indentStore = db.createObjectStore('indents', {
                        keyPath: 'id',
                    });
                    indentStore.createIndex('siding_id', 'siding_id', {
                        unique: false,
                    });
                    indentStore.createIndex('indent_number', 'indent_number', {
                        unique: true,
                    });
                }

                if (!db.objectStoreNames.contains('coal_stock')) {
                    const stockStore = db.createObjectStore('coal_stock', {
                        keyPath: 'id',
                    });
                    stockStore.createIndex('siding_id', 'siding_id', {
                        unique: false,
                    });
                    stockStore.createIndex('as_of_date', 'as_of_date', {
                        unique: false,
                    });
                }
