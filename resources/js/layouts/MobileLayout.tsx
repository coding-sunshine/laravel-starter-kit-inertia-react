        ...
        <div className="flex min-h-screen flex-col bg-white">
            ...
            <main
                className={`flex-1 ${
                    noPadding
                        ? 'w-full'
                        : 'w-full px-3 py-4 sm:px-4 sm:py-6 md:p-6 md:px-6'
                }`}
            >
                {children}
            </main>
...
export function MobileSafeArea({
    ...
    return (
        <div
            className={`w-full ${top ? 'pt-safe-top' : ''} ${
...
export function MobileListItem({
    ...
    return (
        <button
            onClick={onPress}
            className="flex min-h-[56px] w-full items-center gap-3 border-b border-gray-100 bg-white px-4 py-3 text-left transition-colors hover:bg-gray-50 active:bg-gray-100 disabled:opacity-50"
            disabled={!onPress}
        >
            {icon && (
            ...
            {badge !== undefined && badge > 0 && (
                <span className="inline-flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full bg-red-500 text-xs font-bold text-white">
                    {badge > 9 ? '9+' : badge}
                </span>
            )}
            {rightElement && (
