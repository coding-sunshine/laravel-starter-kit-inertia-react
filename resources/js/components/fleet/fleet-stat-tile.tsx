...
/**
 * KPI tint map (reference StatTile) — border, glow, badge, iconCircle.
 */
...
> = {
    blue: {
        ...
        iconCircle: 'bg-slate-100/70 dark:bg-white/10 text-slate-600 dark:text-slate-300',
    },
    violet: {
        border: 'border-l-violet-400/70',
        glow: 'bg-violet-400/20',
        badge: 'bg-violet-500/15 text-violet-600 dark:text-violet-400 border-violet-200/60',
        iconCircle: 'bg-indigo-100/70 dark:bg-white/10 text-indigo-600 dark:text-indigo-300',
    },
    emerald: {
        border: 'border-l-emerald-400/70',
        glow: 'bg-emerald-400/20',
        badge: 'bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 border-emerald-200/60',
        iconCircle: 'bg-emerald-100/70 dark:bg-white/10 text-emerald-600 dark:text-emerald-300',
    },
    slate: {
    ...
    amber: {
        ...
        iconCircle: 'bg-amber-100/70 dark:bg-white/10 text-amber-600 dark:text-amber-300',
    },
    sky: {
        border: 'border-l-sky-400/70',
        glow: 'bg-sky-400/20',
        badge: 'bg-sky-500/15 text-sky-600 dark:text-sky-400 border-sky-200/60',
        iconCircle: 'bg-slate-100/70 dark:bg-white/10 text-slate-600 dark:text-sky-300',
    },
    rose: {
        border: 'border-l-rose-400/70',
        glow: 'bg-rose-400/20',
        badge: 'bg-rose-500/15 text-rose-600 dark:text-rose-400 border-rose-200/60',
        iconCircle: 'bg-rose-100/70 dark:bg-white/10 text-rose-600 dark:text-rose-300',
    },
};
...
export function FleetStatTile({
    ...
    return (
        <FleetGlassCard
            ...
            noPadding
        >
            <div className={cn('absolute -left-5 -top-5 h-12 w-12 rounded-full blur-xl', glow)} aria-hidden />
            <div className="relative flex items-center gap-2">
                <div
                    className={cn(
                        'flex shrink-0 items-center justify-center',
                        isCockpit ? 'h-8 w-8 rounded-full' : 'h-6 w-6 rounded-md border',
                ...
                <p
                    className={cn(
                        'truncate font-medium uppercase tracking-wide leading-tight text-muted-foreground',
            ...
            <div className="relative mt-1">
                <p className={cn('font-semibold tracking-tight text-foreground', isCockpit ? 'text-2xl leading-none' : 'text-xl')}>
                    {value}
                </p>
                <p className={cn('mt-0.5 flex items-center leading-tight text-muted-foreground', isCockpit ? 'text-xs' : 'text-[10px]')}>
