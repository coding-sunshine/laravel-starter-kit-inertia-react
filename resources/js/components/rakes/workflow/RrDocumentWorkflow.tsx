...
export function RrDocumentWorkflow({ rake, disabled }: RrDocumentWorkflowProps) {
    ...
    /** Diverted with legs: only legs must be complete (primary may be absent). Diverted with no legs yet: primary only. Non-diverted: primary only. */
    const allComplete = isDiverted
        ? diverrtDestinations.length > 0
            ? diversionSlotsComplete
            : hasPrimary
        : hasPrimary;
    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>
                    {isDiverted
                        ? 'Upload the primary RR (optional if only diversion legs apply) plus one RR per diversion leg (Station To on each PDF must match the leg IR code).'
            ...
            <CardContent className="space-y-6">
                ...
                {isDiverted && !disabled && (
                    <div className="space-y-3">
                        ...
                        <p className="text-muted-foreground text-xs">
                            Add one row per diverted destination. Use the IR <strong>Station To</strong> code as shown on
                            that leg&apos;s RR PDF.
                        </p>
