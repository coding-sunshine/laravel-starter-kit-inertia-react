...
interface Props {
    fines: { data: FineRecord[]; links: { url: string | null; label: string; active: boolean }[] };
...
export default function FleetFinesIndex({ fines, filters, vehicles, drivers, fineTypes, statuses }: Props) {
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <div className="flex items-center justify-between">
                    ...
                    <Button asChild>
                        <Link href="/fleet/fines/create"><Plus className="mr-2 size-4" />New</Link>
                    </Button>
                </div>
                <form method="get" className="flex flex-wrap items-end gap-4 rounded-lg border p-4">
                    ...
                    <div className="space-y-1">
                        <Label>Driver</Label>
                        <select name="driver_id" defaultValue={filters.driver_id ?? ''} className="h-9 w-40 rounded-md border border-input bg-transparent px-3 py-1 text-sm">
                            <option value="">All</option>
                ...
                {fines.data.length === 0 ? (
                    <div className="rounded-lg border border-dashed py-16 text-center">
                        ...
                        <p className="mt-2 text-sm text-muted-foreground">No fines recorded.</p>
                        <Button asChild className="mt-4"><Link href="/fleet/fines/create">Add fine</Link></Button>
                    </div>
                ) : (
                    <>
                                ...
                                <tbody>
                                    {fines.data.map((row) => (
                                        <tr key={row.id} className="border-b last:border-0">
                                            <td className="p-3">{row.fine_type}</td>
                                            <td className="p-3">{row.vehicle?.registration ?? '—'}</td>
                                            <td className="p-3">{row.driver ? `${row.driver.first_name} ${row.driver.last_name}` : '—'}</td>
                                            <td className="p-3">{new Date(row.offence_date).toLocaleDateString()}</td>
                                            <td className="p-3">{row.amount}</td>
                                            <td className="p-3">{row.status}</td>
                                            <td className="p-3 text-right">
                                                <Button variant="outline" size="sm" asChild><Link href={`/fleet/fines/${row.id}`}>View</Link></Button>
                                                <Button variant="outline" size="sm" asChild><Link href={`/fleet/fines/${row.id}/edit`}><Pencil className="ml-1 size-3.5" /></Link></Button>
                                                <Form action={`/fleet/fines/${row.id}`} method="delete" className="ml-2 inline" onSubmit={(e) => { if (!confirm('Delete?')) e.preventDefault(); }}>
