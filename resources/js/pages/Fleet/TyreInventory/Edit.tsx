...
export default function FleetTyreInventoryEdit({ tyreInventory }: Props) {
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: 'Edit', href: `/fleet/tyre-inventory/${tyreInventory.id}/edit` },
    ];
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <div className="flex items-center gap-4">
                    <Button variant="ghost" size="sm" asChild><Link href={`/fleet/tyre-inventory/${tyreInventory.id}`}>Back</Link></Button>
                    <h1 className="text-2xl font-semibold">Edit tyre inventory item</h1>
                </div>
                <form onSubmit={(e) => { e.preventDefault(); form.put(`/fleet/tyre-inventory/${tyreInventory.id}`); }} className="max-w-xl space-y-4 rounded-lg border p-6">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button type="button" variant="outline" asChild><Link href={`/fleet/tyre-inventory/${tyreInventory.id}`}>Cancel</Link></Button>
                    </div>
                </form>
            </div>
        </AppLayout>
