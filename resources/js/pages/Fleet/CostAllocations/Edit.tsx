...
interface CostAllocation { id: number; cost_center_id: number; allocation_date: string; cost_type: string; source_type: string; amount: string; vat_amount: string; approval_status: string; }
interface Props {
...
export default function FleetCostAllocationsEdit({ costAllocation, costCenters, costTypes, sourceTypes, approvalStatuses }: Props) {
    const form = useForm({
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: 'Edit', href: `/fleet/cost-allocations/${costAllocation.id}/edit` },
    ];
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <div className="flex items-center gap-4">
                    <Button variant="ghost" size="sm" asChild><Link href={`/fleet/cost-allocations/${costAllocation.id}`}>Back</Link></Button>
                    <h1 className="text-2xl font-semibold">Edit cost allocation</h1>
                </div>
                <form onSubmit={(e) => { e.preventDefault(); form.put(`/fleet/cost-allocations/${costAllocation.id}`); }} className="max-w-xl space-y-4 rounded-lg border p-6">
                    <div className="space-y-2">
                        <Label>Cost center</Label>
                        <select required value={form.data.cost_center_id} onChange={e => form.setData('cost_center_id', Number(e.target.value))} className="h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button type="button" variant="outline" asChild><Link href={`/fleet/cost-allocations/${costAllocation.id}`}>Cancel</Link></Button>
                    </div>
                </form>
            </div>
        </AppLayout>
