...
interface FuelTransactionRecord { id: number; vehicle_id: number; fuel_card_id: number; transaction_timestamp: string; fuel_type: string; price_per_litre: number; total_cost: number; }
interface Props {
...
export default function FleetFuelTransactionsEdit({ fuelTransaction, fuelTypes, vehicles, fuelCards }: Props) {
    const form = useForm({
        vehicle_id: fuelTransaction.vehicle_id,
        fuel_card_id: fuelTransaction.fuel_card_id,
        transaction_timestamp: fuelTransaction.transaction_timestamp ? new Date(fuelTransaction.transaction_timestamp).toISOString().slice(0, 16) : '',
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: `#${fuelTransaction.id}`, href: `/fleet/fuel-transactions/${fuelTransaction.id}` },
        { title: 'Edit', href: `/fleet/fuel-transactions/${fuelTransaction.id}/edit` },
    ];
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        form.transform((d) => ({
            ...d,
            price_per_litre: d.price_per_litre === '' ? 0 : Number(d.price_per_litre),
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Fleet – Edit fuel transaction #${fuelTransaction.id}`} />
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <h1 className="text-2xl font-semibold">Edit fuel transaction</h1>
                <form onSubmit={handleSubmit} className="max-w-xl space-y-4">
                    <div>
                        <Label htmlFor="vehicle_id">Vehicle *</Label>
                        <select id="vehicle_id" value={data.vehicle_id} onChange={(e) => setData('vehicle_id', Number(e.target.value))} className="mt-1 flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm" required>
                    ...
                    <div>
                        ...
                        <select id="fuel_card_id" value={data.fuel_card_id} onChange={(e) => setData('fuel_card_id', Number(e.target.value))} className="mt-1 flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm" required>
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button variant="outline" asChild><Link href={`/fleet/fuel-transactions/${fuelTransaction.id}`}>Cancel</Link></Button>
                    </div>
                </form>
            </div>
        </AppLayout>
