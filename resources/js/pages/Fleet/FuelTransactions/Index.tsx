...
interface Props {
    fuelTransactions: { data: TxRecord[]; links: { url: string | null; label: string; active: boolean }[] };
    filters: Record<string, string>;
    vehicles: { id: number; registration: string }[];
    fuelCards: { id: number; card_number: string }[];
}

export default function FleetFuelTransactionsIndex({ fuelTransactions, filters, vehicles, fuelCards }: Props) {
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <div className="flex items-center justify-between">
                    <h1 className="text-2xl font-semibold">Fuel transactions</h1>
                    <Button asChild>
                        <Link href="/fleet/fuel-transactions/create"><Plus className="mr-2 size-4" />New</Link>
                    </Button>
                </div>
                <form method="get" className="flex flex-wrap items-end gap-4 rounded-lg border p-4">
                    ...
                    <div className="space-y-1">
                        ...
                        <select name="fuel_card_id" defaultValue={filters.fuel_card_id ?? ''} className="h-9 w-40 rounded-md border border-input bg-transparent px-3 py-1 text-sm">
                            <option value="">All</option>
                ...
                {fuelTransactions.data.length === 0 ? (
                    <div className="rounded-lg border border-dashed py-16 text-center">
                        ...
                        <p className="mt-2 text-sm text-muted-foreground">No fuel transactions yet.</p>
                        <Button asChild className="mt-4"><Link href="/fleet/fuel-transactions/create">Add transaction</Link></Button>
                    </div>
                ) : (
                    <>
                                ...
                                <tbody>
                                    {fuelTransactions.data.map((row) => (
                                        <tr key={row.id} className="border-b last:border-0">
                                            <td className="p-3">{new Date(row.transaction_timestamp).toLocaleString()}</td>
                                            <td className="p-3">{row.vehicle?.registration ?? '—'}</td>
                                            <td className="p-3">{row.fuel_type}</td>
                                            <td className="p-3">{row.total_cost}</td>
                                            <td className="p-3 text-right">
                                                <Button variant="outline" size="sm" asChild><Link href={`/fleet/fuel-transactions/${row.id}`}>View</Link></Button>
                                                <Button variant="outline" size="sm" asChild><Link href={`/fleet/fuel-transactions/${row.id}/edit`}><Pencil className="ml-1 size-3.5" /></Link></Button>
                                                <Form action={`/fleet/fuel-transactions/${row.id}`} method="delete" className="ml-2 inline" onSubmit={(e) => { if (!confirm('Delete?')) e.preventDefault(); }}>
