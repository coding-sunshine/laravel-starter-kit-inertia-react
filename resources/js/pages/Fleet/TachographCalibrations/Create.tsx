...
export default function TachographCalibrationsCreate({ vehicles, telematicsDevices, statuses }: Props) {
    const form = useForm({
        vehicle_id: '' as number | '',
        telematics_device_id: '' as number | '',
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: 'New', href: '/fleet/tachograph-calibrations/create' },
    ];
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Fleet – New tachograph calibration" />
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <div className="flex items-center gap-4">
                    <Button variant="ghost" size="sm" asChild><Link href="/fleet/tachograph-calibrations">Back</Link></Button>
                    <h1 className="text-2xl font-semibold">New tachograph calibration</h1>
                </div>
                <form onSubmit={(e) => { e.preventDefault(); form.post('/fleet/tachograph-calibrations'); }} className="max-w-xl space-y-4 rounded-lg border p-6">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button type="button" variant="outline" asChild><Link href="/fleet/tachograph-calibrations">Cancel</Link></Button>
                    </div>
                </form>
            </div>
        </AppLayout>
