...
export default function SafetyObservationsCreate({ users, locations, categories, statuses }: Props) {
    const form = useForm({
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: 'New', href: '/fleet/safety-observations/create' },
    ];
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Fleet – New safety observation" />
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <div className="flex items-center gap-4">
                    <Button variant="ghost" size="sm" asChild><Link href="/fleet/safety-observations">Back</Link></Button>
                    <h1 className="text-2xl font-semibold">New safety observation</h1>
                </div>
                <form onSubmit={(e) => { e.preventDefault(); form.post('/fleet/safety-observations'); }} className="max-w-xl space-y-4 rounded-lg border p-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <select required value={form.data.reported_by} onChange={e => form.setData('reported_by', e.target.value ? Number(e.target.value) : '')} className="h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm">
                            <option value="">—</option>
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button type="button" variant="outline" asChild><Link href="/fleet/safety-observations">Cancel</Link></Button>
                    </div>
                </form>
            </div>
        </AppLayout>
