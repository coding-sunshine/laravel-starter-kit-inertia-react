...
interface Props {
    workshopBays: { data: Row[]; links: { url: string | null; label: string; active: boolean }[] };
    statuses: { value: string; name: string }[];
}

export default function FleetWorkshopBaysIndex({ workshopBays, statuses }: Props) {
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <div className="flex items-center justify-between">
                    ...
                    <Button asChild>
                        <Link href="/fleet/workshop-bays/create"><Plus className="mr-2 size-4" />New</Link>
                    </Button>
                </div>
                {workshopBays.data.length === 0 ? (
                    <div className="rounded-lg border border-dashed py-16 text-center">
                        <Wrench className="mx-auto size-10 text-muted-foreground" />
                        <p className="mt-2 text-sm text-muted-foreground">No workshop bays yet.</p>
                        <Button asChild className="mt-4"><Link href="/fleet/workshop-bays/create">Add</Link></Button>
                    </div>
                ) : (
                    <>
                                    ...
                                    {workshopBays.data.map((row) => (
                                        <tr key={row.id} className="border-b last:border-0">
                                            ...
                                            <td className="p-3 text-right">
                                                <Button variant="outline" size="sm" asChild><Link href={`/fleet/workshop-bays/${row.id}`}>View</Link></Button>
                                                <Button variant="outline" size="sm" asChild><Link href={`/fleet/workshop-bays/${row.id}/edit`}><Pencil className="ml-1 size-3.5" /></Link></Button>
                                                <Form action={`/fleet/workshop-bays/${row.id}`} method="delete" className="ml-2 inline" onSubmit={(e) => { if (!confirm('Delete?')) e.preventDefault(); }}>
