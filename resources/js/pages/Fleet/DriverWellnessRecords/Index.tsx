...
interface Props {
    driverWellnessRecords: { data: Row[]; links: { url: string | null; label: string; active: boolean }[] };
...
export default function FleetDriverWellnessRecordsIndex(props: Props) {
    const { driverWellnessRecords, filters, drivers, sleepQualities } = props;
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <div className="flex items-center justify-between">
                    <h1 className="text-2xl font-semibold">Driver wellness records</h1>
                    <Button asChild><Link href="/fleet/driver-wellness-records/create"><Plus className="mr-2 size-4" />New</Link></Button>
                </div>
                ...
                {driverWellnessRecords.data.length === 0 ? (
                    <div className="rounded-lg border border-dashed py-16 text-center">
                        ...
                        <p className="mt-2 text-sm text-muted-foreground">No wellness records yet.</p>
                        <Button asChild className="mt-4"><Link href="/fleet/driver-wellness-records/create">Add record</Link></Button>
                    </div>
                ) : (
                    <>
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b bg-muted/50">
                                        ...
                                        <th className="p-3 text-left font-medium">Fatigue</th>
                                        <th className="p-3 text-left font-medium">Rest (h)</th>
                                ...
                                <tbody>
                                    {driverWellnessRecords.data.map((row) => (
                                        <tr key={row.id} className="border-b last:border-0">
                                            ...
                                            <td className="p-3">{row.fatigue_level ?? '—'}</td>
                                            <td className="p-3">{row.rest_hours ?? '—'}</td>
                                            <td className="p-3 text-right">
                                                <Button variant="outline" size="sm" asChild><Link href={`/fleet/driver-wellness-records/${row.id}`}>View</Link></Button>
                                                <Button variant="outline" size="sm" asChild><Link href={`/fleet/driver-wellness-records/${row.id}/edit`}><Pencil className="ml-1 size-3.5" /></Link></Button>
                                                <Form action={`/fleet/driver-wellness-records/${row.id}`} method="delete" className="ml-2 inline" onSubmit={(e) => { if (!confirm('Delete?')) e.preventDefault(); }}>
