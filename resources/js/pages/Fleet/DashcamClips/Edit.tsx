...
interface DashcamClip { id: number; vehicle_id: number; driver_id?: number; incident_id?: number; event_type: string; status: string; recorded_at: string; }
interface Props {
...
export default function FleetDashcamClipsEdit({ dashcamClip, vehicles, drivers, eventTypes, statuses }: Props) {
    const form = useForm({
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <div className="flex items-center gap-4">
                    <Button variant="ghost" size="sm" asChild><Link href={`/fleet/dashcam-clips/${dashcamClip.id}`}>Back</Link></Button>
                    <h1 className="text-2xl font-semibold">Edit dashcam clip</h1>
                </div>
                <form onSubmit={(e) => { e.preventDefault(); form.put(`/fleet/dashcam-clips/${dashcamClip.id}`); }} className="max-w-xl space-y-4 rounded-lg border p-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <select value={form.data.event_type} onChange={e => form.setData('event_type', e.target.value)} className="h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm">
                            {eventTypes.map((t) => <option key={t.value} value={t.value}>{t.name}</option>)}
                        </select>
                    </div>
                    <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button type="button" variant="outline" asChild><Link href={`/fleet/dashcam-clips/${dashcamClip.id}`}>Cancel</Link></Button>
                    </div>
                </form>
            </div>
        </AppLayout>
