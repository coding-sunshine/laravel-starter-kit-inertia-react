                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                        <Button variant="outline" size="sm" asChild>
                            <Link
                                href={`/fleet/dashcam-clips/${dashcamClip.id}/edit`}
                            >
                                Edit
                            </Link>
                        </Button>
                        <Button variant="ghost" size="sm" asChild>
                            <Link href="/fleet/dashcam-clips">
                                Back to list
                            </Link>
                        </Button>
                    </div>
                ...
                <Card>
                    ...
                    <CardContent className="space-y-2 text-sm">
                        ...
                        <p>
                            ...
                            {new Date(dashcamClip.recorded_at).toLocaleString()}
                        </p>
                        {dashcamClip.vehicle && (
                            <p>
                                <span className="font-medium">Vehicle:</span>{' '}
                                {dashcamClip.vehicle.registration}
                            </p>
                        )}
                        {dashcamClip.driver && (
                            <p>
                                <span className="font-medium">Driver:</span>{' '}
                                {dashcamClip.driver.first_name}{' '}
                                {dashcamClip.driver.last_name}
                            </p>
                        )}
                        {dashcamClip.incident && (
                            <p>
                                <span className="font-medium">Incident:</span>{' '}
                                {dashcamClip.incident.incident_number}
                            </p>
                        )}
                        {dashcamClip.clip_url && (
                            <p>
                                <span className="font-medium">Clip URL:</span>{' '}
                                <a
                                    href={dashcamClip.clip_url}
                                    target="_blank"
