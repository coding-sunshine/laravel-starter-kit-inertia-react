...
interface Props {
    contractorCompliance: { data: Row[]; links: { url: string | null; label: string; active: boolean }[] };
    contractors: { id: number; name: string }[];
    complianceTypes: { value: string; name: string }[];
    statuses: { value: string; name: string }[];
}

export default function FleetContractorComplianceIndex({ contractorCompliance, contractors, complianceTypes, statuses }: Props) {
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <div className="flex items-center justify-between">
                    ...
                    <Button asChild>
                        <Link href="/fleet/contractor-compliance/create"><Plus className="mr-2 size-4" />New</Link>
                    </Button>
                </div>
                {contractorCompliance.data.length === 0 ? (
                    <div className="rounded-lg border border-dashed py-16 text-center">
                        <FileCheck className="mx-auto size-10 text-muted-foreground" />
                        <p className="mt-2 text-sm text-muted-foreground">No contractor compliance records yet.</p>
                        <Button asChild className="mt-4"><Link href="/fleet/contractor-compliance/create">Add</Link></Button>
                    </div>
                ) : (
                    <>
                                ...
                                <tbody>
                                    {contractorCompliance.data.map((row) => (
                                        <tr key={row.id} className="border-b last:border-0">
                                            ...
                                            <td className="p-3">{complianceTypes.find(c => c.value === row.compliance_type)?.name ?? row.compliance_type}</td>
                                            ...
                                            <td className="p-3 text-right">
                                                <Button variant="outline" size="sm" asChild><Link href={`/fleet/contractor-compliance/${row.id}`}>View</Link></Button>
                                                <Button variant="outline" size="sm" asChild><Link href={`/fleet/contractor-compliance/${row.id}/edit`}><Pencil className="ml-1 size-3.5" /></Link></Button>
                                                <Form action={`/fleet/contractor-compliance/${row.id}`} method="delete" className="ml-2 inline" onSubmit={(e) => { if (!confirm('Delete?')) e.preventDefault(); }}>
