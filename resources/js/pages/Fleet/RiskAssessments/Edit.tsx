...
interface Assessment { id: number; subject_type: string; subject_id: number; title: string; type: string; reference_number?: string; description?: string; hazards?: string; control_measures?: string; status: string; review_date?: string; }
interface Props {
...
export default function RiskAssessmentsEdit({ riskAssessment, types, statuses, subjectOptions }: Props) {
    const form = useForm({
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: 'Edit', href: `/fleet/risk-assessments/${riskAssessment.id}/edit` },
    ];
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <div className="flex items-center gap-4">
                    <Button variant="ghost" size="sm" asChild><Link href={`/fleet/risk-assessments/${riskAssessment.id}`}>Back</Link></Button>
                    <h1 className="text-2xl font-semibold">Edit risk assessment</h1>
                </div>
                <form onSubmit={(e) => { e.preventDefault(); form.put(`/fleet/risk-assessments/${riskAssessment.id}`); }} className="max-w-xl space-y-4 rounded-lg border p-6">
                    <div className="space-y-2">
                        <Label>Subject *</Label>
                        <select required value={subjectValue} onChange={e => onSubjectChange(e.target.value)} className="h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button type="button" variant="outline" asChild><Link href={`/fleet/risk-assessments/${riskAssessment.id}`}>Cancel</Link></Button>
                    </div>
                </form>
            </div>
        </AppLayout>
