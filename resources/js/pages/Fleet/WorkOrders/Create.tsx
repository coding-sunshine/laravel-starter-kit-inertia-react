...
export default function FleetWorkOrdersCreate({ workOrderNumber, workTypes, priorities, statuses, vehicles, garages }: Props) {
    const form = useForm({
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                ...
                <form onSubmit={handleSubmit} className="max-w-xl space-y-4">
                    <div>
                        ...
                        <Input id="work_order_number" value={data.work_order_number} onChange={(e) => setData('work_order_number', e.target.value)} className="mt-1" />
                        {errors.work_order_number && <p className="mt-1 text-sm text-destructive">{errors.work_order_number}</p>}
                    </div>
                    <div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select id="work_type" value={data.work_type} onChange={(e) => setData('work_type', e.target.value)} className="mt-1 flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm">
                                {workTypes.map((o) => <option key={o.value} value={o.value}>{o.name}</option>)}
                            </select>
                        </div>
                        <div>
                            <Label htmlFor="priority">Priority *</Label>
                            <select id="priority" value={data.priority} onChange={(e) => setData('priority', e.target.value)} className="mt-1 flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm">
                                {priorities.map((o) => <option key={o.value} value={o.value}>{o.name}</option>)}
                            </select>
                        </div>
                    </div>
                    <div>
                    ...
                    <div className="flex gap-2">
                        <Button type="submit" disabled={processing}>Create work order</Button>
                        <Button variant="outline" asChild><Link href="/fleet/work-orders">Cancel</Link></Button>
                    </div>
                </form>
            </div>
        </AppLayout>
