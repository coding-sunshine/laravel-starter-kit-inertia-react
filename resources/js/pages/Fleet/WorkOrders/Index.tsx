        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <div className="flex items-center justify-between">
                    ...
                    <Button asChild>
                        <Link href="/fleet/work-orders/create"><Plus className="mr-2 size-4" />New</Link>
                    </Button>
                </div>
                ...
                {workOrders.data.length === 0 ? (
                    <div className="rounded-lg border border-dashed py-16 text-center">
                        ...
                        <Button asChild className="mt-4"><Link href="/fleet/work-orders/create">Create work order</Link></Button>
                    </div>
                ) : (
                    <>
                                ...
                                <tbody>
                                    {workOrders.data.map((row) => (
                                        <tr key={row.id} className="border-b last:border-0">
                                            <td className="p-3"><Link href={`/fleet/work-orders/${row.id}`} className="font-medium hover:underline">{row.work_order_number}</Link></td>
                                            <td className="p-3">{row.title}</td>
                                            <td className="p-3">{row.vehicle?.registration ?? '—'}</td>
                                            <td className="p-3">{row.status}</td>
                                            <td className="p-3">{row.priority}</td>
                                            <td className="p-3">{row.scheduled_date ? new Date(row.scheduled_date).toLocaleDateString() : '—'}</td>
                                            <td className="p-3 text-right">
                                                <Button variant="outline" size="sm" asChild><Link href={`/fleet/work-orders/${row.id}`}>View</Link></Button>
                                                <Button variant="outline" size="sm" asChild><Link href={`/fleet/work-orders/${row.id}/edit`}><Pencil className="ml-1 size-3.5" /></Link></Button>
                                                <Form action={`/fleet/work-orders/${row.id}`} method="delete" className="ml-2 inline" onSubmit={(e) => { if (!confirm('Delete?')) e.preventDefault(); }}>
