...
export default function FleetWorkOrderLinesIndex({ workOrder, workOrderLines }: Props) {
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: 'Lines', href: `/fleet/work-orders/${workOrder.id}/work-order-lines` },
    ];
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <div className="flex flex-wrap items-center justify-between gap-4">
                    <h1 className="text-2xl font-semibold">Work order lines – {workOrder.work_order_number}</h1>
                    <div className="flex gap-2">
                        <Button asChild><Link href={`/fleet/work-orders/${workOrder.id}/work-order-lines/create`}><Plus className="mr-2 size-4" />Add line</Link></Button>
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="border-b bg-muted/50">
                                    ...
                                    <th className="p-3 text-left font-medium">Description / Part</th>
                            ...
                            <tbody>
                                {workOrderLines.map((line) => (
                                    <tr key={line.id} className="border-b last:border-0">
                                        ...
                                        <td className="p-3">{line.description ?? line.partsInventory?.part_number ?? '—'}</td>
                                        ...
                                        <td className="p-3 text-right">
                                            ...
                                            <Form action={`/fleet/work-orders/${workOrder.id}/work-order-lines/${line.id}`} method="delete" className="ml-2 inline" onSubmit={(e) => { if (!confirm('Delete?')) e.preventDefault(); }}>
