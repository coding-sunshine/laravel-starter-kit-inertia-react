...
export default function FleetWorkOrderPartsIndex({ workOrder, workOrderParts }: Props) {
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: 'Parts', href: `/fleet/work-orders/${workOrder.id}/work-order-parts` },
    ];
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <div className="flex flex-wrap items-center justify-between gap-4">
                    <h1 className="text-2xl font-semibold">Work order parts – {workOrder.work_order_number}</h1>
                    <div className="flex gap-2">
                        <Button asChild><Link href={`/fleet/work-orders/${workOrder.id}/work-order-parts/create`}><Plus className="mr-2 size-4" />Add part</Link></Button>
                        <Button variant="outline" asChild><Link href={`/fleet/work-orders/${workOrder.id}`}>Back to work order</Link></Button>
                    </div>
                </div>
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="border-b bg-muted/50">
                                    ...
                                    <th className="p-3 text-right font-medium">Qty used</th>
                            ...
                            <tbody>
                                {workOrderParts.map((part) => (
                                    <tr key={part.id} className="border-b last:border-0">
                                        <td className="p-3">{part.partsInventory?.part_number ?? '—'}</td>
                                        <td className="p-3 text-right">{part.quantity_used}</td>
                                        <td className="p-3 text-right">{part.unit_cost ?? '—'}</td>
                                        <td className="p-3 text-right">{part.total_cost ?? '—'}</td>
                                        <td className="p-3 text-right">
                                            ...
                                            <Form action={`/fleet/work-orders/${workOrder.id}/work-order-parts/${part.id}`} method="delete" className="ml-2 inline" onSubmit={(e) => { if (!confirm('Delete?')) e.preventDefault(); }}>
