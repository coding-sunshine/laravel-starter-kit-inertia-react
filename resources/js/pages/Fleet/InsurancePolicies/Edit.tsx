...
export default function FleetInsurancePoliciesEdit({
    insurancePolicy,
    policyTypes,
    coverageTypes,
    statuses,
    ...
    const form = useForm({
        ...
        insurer_name: insurancePolicy.insurer_name,
        policy_type: insurancePolicy.policy_type,
        coverage_type: insurancePolicy.coverage_type,
        start_date:
            typeof insurancePolicy.start_date === 'string'
                ? insurancePolicy.start_date.slice(0, 10)
                : '',
        end_date:
            typeof insurancePolicy.end_date === 'string'
                ? insurancePolicy.end_date.slice(0, 10)
                : '',
        premium_amount:
            insurancePolicy.premium_amount != null ? String(insurancePolicy.premium_amount) : '',
        excess_amount:
            insurancePolicy.excess_amount != null ? String(insurancePolicy.excess_amount) : '',
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: insurancePolicy.policy_number, href: `/fleet/insurance-policies/${insurancePolicy.id}` },
        { title: 'Edit', href: `/fleet/insurance-policies/${insurancePolicy.id}/edit` },
    ];
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        form.put(`/fleet/insurance-policies/${insurancePolicy.id}`);
    };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Fleet – Edit ${insurancePolicy.policy_number}`} />
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <h1 className="text-2xl font-semibold">Edit insurance policy</h1>
                <form onSubmit={handleSubmit} className="max-w-xl space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <Input
                                ...
                                onChange={(e) => setData('excess_amount', e.target.value)}
                                className="mt-1"
                            />
                        </div>
                    </div>
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button variant="outline" asChild>
                            <Link href={`/fleet/insurance-policies/${insurancePolicy.id}`}>Cancel</Link>
                        </Button>
                    </div>
                </form>
            </div>
