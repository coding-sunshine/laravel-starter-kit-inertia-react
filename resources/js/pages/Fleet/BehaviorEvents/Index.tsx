...
interface Props {
    behaviorEvents: { data: EventRecord[]; links: { url: string | null; label: string; active: boolean }[] };
...
export default function FleetBehaviorEventsIndex({ behaviorEvents, filters, vehicles, drivers, eventTypes }: Props) {
    ...
    const applyFilters = (e: React.FormEvent) => {
        ...
        router.get('/fleet/behavior-events', Object.fromEntries(data.entries()) as Record<string, string>, { preserveState: true });
    };
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                ...
                <form onSubmit={applyFilters} className="flex flex-wrap items-end gap-4 rounded-lg border p-4">
                    <div className="space-y-1">
                        ...
                        <select id="vehicle_id" name="vehicle_id" defaultValue={filters.vehicle_id ?? ''} className="h-9 w-40 rounded-md border border-input bg-transparent px-3 py-1 text-sm">
                    ...
                    <div className="space-y-1">
                        ...
                        <select id="driver_id" name="driver_id" defaultValue={filters.driver_id ?? ''} className="h-9 w-40 rounded-md border border-input bg-transparent px-3 py-1 text-sm">
                            <option value="">All</option>
                            {drivers.map((d) => (
                ...
                {behaviorEvents.data.length === 0 ? (
                    <div className="rounded-lg border border-dashed py-16 text-center">
                        ...
                        <p className="mt-2 text-sm text-muted-foreground">No behavior events found.</p>
                    </div>
                ) : (
                    <>
                                        ...
                                        <tr key={row.id} className="border-b last:border-0">
                                            ...
                                            <td className="p-3 text-right">
                                                <Button variant="outline" size="sm" asChild>
                                                    <Link href={`/fleet/behavior-events/${row.id}`}>View</Link>
                                                </Button>
                                            </td>
                                        </tr>
                                    ))}
                        ...
                        {behaviorEvents.links && behaviorEvents.links.length > 1 && (
