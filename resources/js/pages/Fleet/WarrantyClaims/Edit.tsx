...
interface ClaimRecord { id: number; work_order_id: number; claim_number: string; status: string; claim_amount?: string | number | null; settlement_amount?: string | number | null; submitted_date?: string | null; settled_at?: string | null; }
interface Props {
...
export default function FleetWarrantyClaimsEdit({ warrantyClaim, workOrders, statuses }: Props) {
    const form = useForm({
        work_order_id: warrantyClaim.work_order_id,
        claim_number: warrantyClaim.claim_number,
        status: warrantyClaim.status,
        claim_amount: warrantyClaim.claim_amount != null ? String(warrantyClaim.claim_amount) : '',
        settlement_amount: warrantyClaim.settlement_amount != null ? String(warrantyClaim.settlement_amount) : '',
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: 'Edit', href: `/fleet/warranty-claims/${warrantyClaim.id}/edit` },
    ];
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        form.post(`/fleet/warranty-claims/${warrantyClaim.id}`, { forceFormData: true });
    };
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                ...
                <form onSubmit={handleSubmit} className="max-w-xl space-y-4">
                    <div>
                        ...
                        <select value={data.work_order_id} onChange={(e) => setData('work_order_id', Number(e.target.value))} className="mt-1 flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm" required>
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button type="button" variant="outline" asChild><Link href={`/fleet/warranty-claims/${warrantyClaim.id}`}>Cancel</Link></Button>
                    </div>
                </form>
            </div>
        </AppLayout>
