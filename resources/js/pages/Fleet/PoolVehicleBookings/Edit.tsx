...
export default function FleetPoolVehicleBookingsEdit({ poolVehicleBooking, vehicles, users, statuses }: Props) {
    const form = useForm({
        vehicle_id: poolVehicleBooking.vehicle_id,
        user_id: poolVehicleBooking.user_id,
        booking_start: poolVehicleBooking.booking_start?.slice(0, 16) ?? '',
        booking_end: poolVehicleBooking.booking_end?.slice(0, 16) ?? '',
        status: poolVehicleBooking.status ?? 'confirmed',
        purpose: poolVehicleBooking.purpose ?? '',
        destination: poolVehicleBooking.destination ?? '',
        odometer_start: poolVehicleBooking.odometer_start ?? ('' as number | ''),
        odometer_end: poolVehicleBooking.odometer_end ?? ('' as number | ''),
    });
    const breadcrumbs: BreadcrumbItem[] = [
        { title: 'Dashboard', href: dashboard().url },
        { title: 'Fleet', href: '/fleet' },
        { title: 'Pool vehicle bookings', href: '/fleet/pool-vehicle-bookings' },
        { title: 'Edit', href: `/fleet/pool-vehicle-bookings/${poolVehicleBooking.id}/edit` },
    ];
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Fleet – Edit pool vehicle booking" />
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <div className="flex items-center gap-4">
                    <Button variant="ghost" size="sm" asChild><Link href={`/fleet/pool-vehicle-bookings/${poolVehicleBooking.id}`}>Back</Link></Button>
                    <h1 className="text-2xl font-semibold">Edit pool vehicle booking</h1>
                </div>
                <form onSubmit={(e) => { e.preventDefault(); form.put(`/fleet/pool-vehicle-bookings/${poolVehicleBooking.id}`); }} className="max-w-xl space-y-4 rounded-lg border p-6">
                    ...
                    <div className="space-y-2">
                        <Label>User</Label>
                        <select required value={form.data.user_id} onChange={e => form.setData('user_id', Number(e.target.value))} className="h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button type="button" variant="outline" asChild><Link href={`/fleet/pool-vehicle-bookings/${poolVehicleBooking.id}`}>Cancel</Link></Button>
                    </div>
                </form>
            </div>
        </AppLayout>
