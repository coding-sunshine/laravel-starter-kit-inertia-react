...
export default function VehicleCheckTemplatesEdit({ vehicleCheckTemplate, checkTypes }: Props) {
    const form = useForm({
        ...
        completion_percentage_threshold: (vehicleCheckTemplate.completion_percentage_threshold ?? '') as number | '',
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: 'Edit', href: `/fleet/vehicle-check-templates/${vehicleCheckTemplate.id}/edit` },
    ];
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <div className="flex items-center gap-4">
                    <Button variant="ghost" size="sm" asChild><Link href={`/fleet/vehicle-check-templates/${vehicleCheckTemplate.id}`}>Back</Link></Button>
                    <h1 className="text-2xl font-semibold">Edit vehicle check template</h1>
                </div>
                <form onSubmit={(e) => { e.preventDefault(); form.put(`/fleet/vehicle-check-templates/${vehicleCheckTemplate.id}`); }} className="max-w-xl space-y-4 rounded-lg border p-6">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button type="button" variant="outline" asChild><Link href={`/fleet/vehicle-check-templates/${vehicleCheckTemplate.id}`}>Cancel</Link></Button>
                    </div>
                </form>
            </div>
        </AppLayout>
