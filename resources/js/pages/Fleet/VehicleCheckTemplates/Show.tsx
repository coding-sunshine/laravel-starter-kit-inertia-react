...
export default function VehicleCheckTemplatesShow({
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        {
            title: vehicleCheckTemplate.name,
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button variant="ghost" size="sm" asChild>
                            <Link href="/fleet/vehicle-check-templates">
                                Back
                            </Link>
                        </Button>
                    </div>
                ...
                <dl className="max-w-md space-y-2 rounded-lg border p-6">
                    ...
                    <div>
                        ...
                        <dd>{vehicleCheckTemplate.code ?? '—'}</dd>
                    </div>
                    <div>
                        <dt className="text-sm text-muted-foreground">
                            Check type
                        </dt>
                        <dd>{vehicleCheckTemplate.check_type}</dd>
                    </div>
                    <div>
                        <dt className="text-sm text-muted-foreground">
                            Category
                        </dt>
                        <dd>{vehicleCheckTemplate.category ?? '—'}</dd>
                    </div>
                    <div>
                        <dt className="text-sm text-muted-foreground">
                            Workflow route
                        </dt>
                        <dd>{vehicleCheckTemplate.workflow_route ?? '—'}</dd>
                    </div>
                    <div>
                        <dt className="text-sm text-muted-foreground">
                        ...
                        <dd>
                            {vehicleCheckTemplate.completion_percentage_threshold ??
                                '—'}
                        </dd>
                    </div>
                    <div>
                        ...
                        <dd>{vehicleCheckTemplate.is_active ? 'Yes' : 'No'}</dd>
                    </div>
                </dl>
            </div>
        </AppLayout>
