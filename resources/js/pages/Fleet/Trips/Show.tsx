...
interface TripRecord {
    ...
    behavior_events?: { id: number; event_type: string; occurred_at: string; severity: string | null }[];
    behaviorEvents?: { id: number; event_type: string; occurred_at: string; severity: string | null }[];
}
interface Props { trip: TripRecord; }
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <div className="flex flex-wrap items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-muted-foreground">
                            Status: {trip.status}
                            {trip.vehicle && <> · Vehicle: <Link href={`/fleet/vehicles/${trip.vehicle.id}`} className="underline">{trip.vehicle.registration}</Link></>}
                            {trip.driver && <> · Driver: <Link href={`/fleet/drivers/${trip.driver.id}`} className="underline">{trip.driver.first_name} {trip.driver.last_name}</Link></>}
                        </p>
                    </div>
                ...
                <div className="grid gap-4 md:grid-cols-2">
                    <Card>
                        <CardHeader className="pb-2"><CardTitle className="text-base">Planned</CardTitle></CardHeader>
                        <CardContent className="text-sm">
                            <p>Start: {trip.planned_start_time ? new Date(trip.planned_start_time).toLocaleString() : '—'}</p>
                    ...
                    <Card>
                        <CardHeader className="pb-2"><CardTitle className="text-base">Actual</CardTitle></CardHeader>
                        <CardContent className="text-sm">
                            <p>Started: {trip.started_at ? new Date(trip.started_at).toLocaleString() : '—'}</p>
                            <p>Ended: {trip.ended_at ? new Date(trip.ended_at).toLocaleString() : '—'}</p>
                            <p>Distance: {trip.distance_km != null ? `${trip.distance_km} km` : '—'}</p>
                            <p>Duration: {trip.duration_minutes != null ? `${trip.duration_minutes} min` : '—'}</p>
                        </CardContent>
                    </Card>
                </div>
                                    ...
                                    <tbody>
                                        {waypoints.map((w) => (
                                            <tr key={w.id} className="border-b last:border-0">
                                                <td className="p-3">{w.sequence}</td>
                                                <td className="p-3">{w.lat}</td>
                                                <td className="p-3">{w.lng}</td>
                                                <td className="p-3">{w.recorded_at ? new Date(w.recorded_at).toLocaleString() : '—'}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                                    ...
                                    <tbody>
                                        {events.map((ev) => (
                                            <tr key={ev.id} className="border-b last:border-0">
                                                ...
                                                <td className="p-3">{ev.severity ?? '—'}</td>
                                                <td className="p-3">{new Date(ev.occurred_at).toLocaleString()}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
