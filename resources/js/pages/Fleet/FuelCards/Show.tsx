                ...
                <Card>
                    ...
                    <CardContent className="space-y-2 text-sm">
                        ...
                        <p>
                            ...
                            {fuelCard.expiry_date
                                ? new Date(
                                      fuelCard.expiry_date,
                                  ).toLocaleDateString()
                                : '—'}
                        </p>
                        {fuelCard.assigned_vehicle && (
                            <p>
                                <span className="font-medium">Vehicle:</span>{' '}
                                <Link
                                    href={`/fleet/vehicles/${fuelCard.assigned_vehicle.id}`}
                                    className="underline"
                                >
                                    {fuelCard.assigned_vehicle.registration}
                                </Link>
                            </p>
                        )}
                        {fuelCard.assigned_driver && (
                            <p>
                                <span className="font-medium">Driver:</span>{' '}
                                <Link
                                    href={`/fleet/drivers/${fuelCard.assigned_driver.id}`}
                                    className="underline"
                                >
                                    {fuelCard.assigned_driver.first_name}{' '}
                                    {fuelCard.assigned_driver.last_name}
                                </Link>
                            </p>
                        )}
                    </CardContent>
