...
export default function FleetCarbonTargetsShow({ carbonTarget }: Props) {
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        {
            title: carbonTarget.name,
            href: `/fleet/carbon-targets/${carbonTarget.id}`,
        },
    ];
    return (
                ...
                <div className="flex flex-wrap items-center justify-between gap-4">
                    ...
                    <div className="flex gap-2">
                        <Button variant="outline" size="sm" asChild>
                            <Link
                                href={`/fleet/carbon-targets/${carbonTarget.id}/edit`}
                            >
                                Edit
                            </Link>
                        </Button>
                ...
                <Card>
                    ...
                    <CardContent className="space-y-2 text-sm">
                        ...
                        <p>
                            ...
                            {carbonTarget.target_year}
                        </p>
                        <p>
                            <span className="font-medium">
                                Target CO₂ (kg):
                            </span>{' '}
                            {String(carbonTarget.target_co2_kg)}
                        </p>
                        {carbonTarget.baseline_co2_kg != null && (
                            <p>
                                <span className="font-medium">
                                    Baseline CO₂ (kg):
                                </span>{' '}
                                {String(carbonTarget.baseline_co2_kg)}
                            </p>
                        )}
                        <p>
                            <span className="font-medium">Active:</span>{' '}
                            {carbonTarget.is_active ? 'Yes' : 'No'}
                        </p>
                    </CardContent>
                </Card>
            </div>
