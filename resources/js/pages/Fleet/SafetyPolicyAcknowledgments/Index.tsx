...
interface Props {
    safetyPolicyAcknowledgments: {
        data: Row[];
        links: { url: string | null; label: string; active: boolean }[];
    };
}

export default function SafetyPolicyAcknowledgmentsIndex({
    safetyPolicyAcknowledgments,
}: Props) {
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <div className="flex items-center justify-between">
                    ...
                    <Button asChild>
                        <Link href="/fleet/safety-policy-acknowledgments/create">
                ...
                {safetyPolicyAcknowledgments.data.length === 0 ? (
                    <div className="rounded-lg border border-dashed py-16 text-center">
                        ...
                        <p className="mt-2 text-sm text-muted-foreground">
                            No safety policy acknowledgments yet.
                        </p>
                    ...
                    <>
                        <div className="rounded-md border">
                            <table className="w-full text-sm">
                                ...
                                <tbody>
                                    {safetyPolicyAcknowledgments.data.map(
                                        (row) => (
                                            <tr
                                                key={row.id}
                        ...
                        {safetyPolicyAcknowledgments.links?.length > 1 && (
                            <div className="flex flex-wrap gap-2">
                                {safetyPolicyAcknowledgments.links.map(
                                    (link, i) => (
                                        <Link
                                            key={i}
                                            href={link.url ?? '#'}
