...
interface WorkflowStep { type?: string; config?: { agent?: string }; }
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                            ...
                            <Button
                                ...
                                onClick={async () => {
                                    ...
                                    try {
                                        ...
                                        if (res.ok && data?.url) router.visit(data.url);
                ...
                <Card>
                    ...
                    <CardContent className="space-y-2 text-sm">
                        ...
                        {workflowDefinition.trigger_type === 'event' && workflowDefinition.trigger_config?.event && (
                            <p>
                                <span className="font-medium">Event:</span>{' '}
                                <code className="rounded bg-muted px-1 text-xs">{workflowDefinition.trigger_config.event}</code>
                            </p>
                        )}
                    </CardContent>
                </Card>
                {workflowDefinition.steps && workflowDefinition.steps.length > 0 && (
                    <Card>
                        <CardHeader className="pb-2">
                            <CardTitle className="text-base">Steps (run in order)</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <ul className="list-inside list-disc space-y-1 text-sm">
                                {workflowDefinition.steps.map((step, i) => (
                                    <li key={i}>
                                        {(step.type === 'run_ai' || step.type === 'ai_agent' || step.type === 'run_ai_job') && step.config?.agent
                                            ? `Run AI: ${step.config.agent}`
                                            : step.type === 'create_alert'
                                            ? 'Create alert (from prior step)'
                                            : step.type === 'create_work_order'
