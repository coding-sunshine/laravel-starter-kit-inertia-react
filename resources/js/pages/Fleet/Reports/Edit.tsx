...
interface Report { id: number; name: string; description?: string; report_type: string; format: string; schedule_frequency: string; is_active: boolean; }
interface Props {
    report: Report;
...
export default function FleetReportsEdit({ report, reportTypes, scheduleFrequencies, formats }: Props) {
    const form = useForm({
        name: report.name,
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <div className="flex items-center gap-4">
                    <Button variant="ghost" size="sm" asChild><Link href={`/fleet/reports/${report.id}`}>Back</Link></Button>
                    <h1 className="text-2xl font-semibold">Edit report</h1>
                </div>
                <form onSubmit={(e) => { e.preventDefault(); form.put(`/fleet/reports/${report.id}`); }} className="max-w-xl space-y-4 rounded-lg border p-6">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button type="button" variant="outline" asChild><Link href={`/fleet/reports/${report.id}`}>Cancel</Link></Button>
                    </div>
                </form>
            </div>
        </AppLayout>
