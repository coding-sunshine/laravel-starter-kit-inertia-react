            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex flex-wrap gap-2">
                        ...
                        <Button variant="outline" size="sm" asChild>
                            <Link href={`/fleet/reports/${report.id}/edit`}>
                                Edit
                            </Link>
                        </Button>
                        <Button variant="ghost" size="sm" asChild>
                            <Link href="/fleet/reports">Back to list</Link>
                        </Button>
                    </div>
                </div>
                <Card>
                ...
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        ...
                        >
                            <Link
                                href={`/fleet/report-executions?report_id=${report.id}`}
                            >
                                View all
                            </Link>
                        </Button>
                    ...
                    <CardContent>
                        {recentExecutions.length === 0 ? (
                            <p className="text-sm text-muted-foreground">
                                No executions yet. Run the report to generate
                                one.
                            </p>
                        ) : (
                            <ul className="space-y-2 text-sm">
                                {recentExecutions.map((ex) => (
                                    ...
                                    >
                                        <Link
                                            href={`/fleet/report-executions/${ex.id}`}
