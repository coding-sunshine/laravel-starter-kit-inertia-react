...
export default function FleetMileageClaimsEdit({ mileageClaim, greyFleetVehicles, users, statuses }: Props) {
    const form = useForm({
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: 'Edit', href: `/fleet/mileage-claims/${mileageClaim.id}/edit` },
    ];
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <div className="flex items-center gap-4">
                    <Button variant="ghost" size="sm" asChild><Link href={`/fleet/mileage-claims/${mileageClaim.id}`}>Back</Link></Button>
                    <h1 className="text-2xl font-semibold">Edit mileage claim</h1>
                </div>
                <form onSubmit={(e) => { e.preventDefault(); form.put(`/fleet/mileage-claims/${mileageClaim.id}`); }} className="max-w-xl space-y-4 rounded-lg border p-6">
                    <div className="space-y-2">
                        <Label>Grey fleet vehicle</Label>
                        <select required value={form.data.grey_fleet_vehicle_id} onChange={e => form.setData('grey_fleet_vehicle_id', Number(e.target.value))} className="h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button type="button" variant="outline" asChild><Link href={`/fleet/mileage-claims/${mileageClaim.id}`}>Cancel</Link></Button>
                    </div>
                </form>
            </div>
        </AppLayout>
