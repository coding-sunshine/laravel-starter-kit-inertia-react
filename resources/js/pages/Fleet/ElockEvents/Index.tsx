...
interface Props {
    elockEvents: { data: Row[]; links: { url: string | null; label: string; active: boolean }[] };
    filters: Record<string, string>;
    vehicles: { id: number; registration: string }[];
    eventTypes: Option[];
}

export default function FleetElockEventsIndex({ elockEvents, filters, vehicles, eventTypes }: Props) {
    ...
    const applyFilters = (e: React.FormEvent) => {
        ...
        router.get('/fleet/e-lock-events', Object.fromEntries(data.entries()) as Record<string, string>, { preserveState: true });
    };
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                ...
                <form onSubmit={applyFilters} className="flex flex-wrap items-end gap-4 rounded-lg border p-4">
                    ...
                    <div className="space-y-1">
                        ...
                        <select name="event_type" defaultValue={filters.event_type ?? ''} className="h-9 w-40 rounded-md border border-input bg-transparent px-3 py-1 text-sm">
                            <option value="">All</option>
                            {eventTypes.map((o) => <option key={o.value} value={o.value}>{o.name}</option>)}
                        </select>
                    </div>
                    <div className="space-y-1">
                ...
                {elockEvents.data.length === 0 ? (
                    <div className="rounded-lg border border-dashed py-16 text-center">
                        ...
                        <p className="mt-2 text-sm text-muted-foreground">No e-lock events found.</p>
                    </div>
                ) : (
                    <>
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b bg-muted/50">
                                        ...
                                        <th className="p-3 text-left font-medium">Alert sent</th>
                                ...
                                <tbody>
                                    {elockEvents.data.map((row) => (
                                        <tr key={row.id} className="border-b last:border-0">
                                            <td className="p-3">{eventTypes.find(t => t.value === row.event_type)?.name ?? row.event_type}</td>
                                            <td className="p-3">{new Date(row.event_timestamp).toLocaleString()}</td>
                                            <td className="p-3">{row.vehicle?.registration ?? '—'}</td>
                                            <td className="p-3">{row.alert_sent ? 'Yes' : 'No'}</td>
                                            <td className="p-3 text-right">
                                                <Button variant="outline" size="sm" asChild><Link href={`/fleet/e-lock-events/${row.id}`}>View</Link></Button>
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
