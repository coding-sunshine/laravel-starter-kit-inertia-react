...
export default function PpeAssignmentsShow({ ppeAssignment }: Props) {
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        {
            title: ppeAssignment.ppe_type,
            href: `/fleet/ppe-assignments/${ppeAssignment.id}`,
        },
    ];
    const assignedTo =
        ppeAssignment.user?.name ??
        (ppeAssignment.driver
            ? ppeAssignment.driver.first_name +
              ' ' +
              ppeAssignment.driver.last_name
            : '—');
    return (
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button variant="ghost" size="sm" asChild>
                            <Link href="/fleet/ppe-assignments">Back</Link>
                        </Button>
                    </div>
                </div>
                <dl className="max-w-md space-y-2 rounded-lg border p-6">
                    ...
                    <div>
                        ...
                        <dd>{ppeAssignment.item_reference ?? '—'}</dd>
                    </div>
                    <div>
                        <dt className="text-sm text-muted-foreground">
                            Issued date
                        </dt>
                        <dd>{ppeAssignment.issued_date}</dd>
                    </div>
                    <div>
                        <dt className="text-sm text-muted-foreground">
                            Expiry / return date
                        </dt>
                        <dd>{ppeAssignment.expiry_or_return_date ?? '—'}</dd>
                    </div>
                    <div>
                        <dt className="text-sm text-muted-foreground">
                            Status
                        </dt>
                        <dd>{ppeAssignment.status}</dd>
                    </div>
                </dl>
            </div>
        </AppLayout>
