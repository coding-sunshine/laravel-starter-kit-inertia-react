...
interface OperatorLicenceRecord {
    ...
    license_type: string;
    traffic_commissioner_area: string;
    issue_date: string;
    effective_date: string;
    expiry_date: string;
    authorized_vehicles: number;
    authorized_trailers: number | null;
    operating_centres: { name: string; address: string }[];
    status: string;
}
interface Props {
    operatorLicence: OperatorLicenceRecord;
    statuses: Option[];
}
...
export default function FleetOperatorLicencesEdit({
    operatorLicence,
    statuses,
}: Props) {
    const { data, setData, put, processing, errors } = useForm({
        license_number: operatorLicence.license_number,
        license_type: operatorLicence.license_type,
        traffic_commissioner_area: operatorLicence.traffic_commissioner_area,
        issue_date: operatorLicence.issue_date,
        effective_date: operatorLicence.effective_date,
        expiry_date: operatorLicence.expiry_date,
        authorized_vehicles: operatorLicence.authorized_vehicles,
        authorized_trailers: (operatorLicence.authorized_trailers ?? '') as
            | number
            | '',
        operating_centres: operatorLicence.operating_centres?.length
            ? operatorLicence.operating_centres
            : [{ name: '', address: '' }],
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        {
            title: operatorLicence.license_number,
            href: `/fleet/operator-licences/${operatorLicence.id}`,
        },
        {
            title: 'Edit',
            href: `/fleet/operator-licences/${operatorLicence.id}/edit`,
        },
    ];
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <h1 className="text-2xl font-semibold">
                    Edit operator licence
                </h1>
                <form
                    onSubmit={(e) => {
                        e.preventDefault();
                        put(`/fleet/operator-licences/${operatorLicence.id}`, {
                            transform,
                        });
                    }}
