...
export default function FleetVehicleTyresEdit({ vehicleTyre, vehicles, tyreInventory, positionOptions }: Props) {
    const form = useForm({
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <div className="flex items-center gap-4">
                    <Button variant="ghost" size="sm" asChild><Link href={`/fleet/vehicle-tyres/${vehicleTyre.id}`}>Back</Link></Button>
                    <h1 className="text-2xl font-semibold">Edit vehicle tyre</h1>
                </div>
                <form onSubmit={(e) => { e.preventDefault(); form.put(`/fleet/vehicle-tyres/${vehicleTyre.id}`); }} className="max-w-xl space-y-4 rounded-lg border p-6">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button type="button" variant="outline" asChild><Link href={`/fleet/vehicle-tyres/${vehicleTyre.id}`}>Cancel</Link></Button>
                    </div>
                </form>
            </div>
        </AppLayout>
