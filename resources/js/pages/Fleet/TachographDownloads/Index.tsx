...
interface Props {
    tachographDownloads: { data: DownloadRecord[]; links: { url: string | null; label: string; active: boolean }[] };
    filters: Record<string, string>;
    drivers: { id: number; first_name: string; last_name: string }[];
    statuses: { value: string; name: string }[];
}

export default function FleetTachographDownloadsIndex({ tachographDownloads, filters, drivers, statuses }: Props) {
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <div className="flex items-center justify-between">
                    <h1 className="text-2xl font-semibold">Tachograph downloads</h1>
                    <Button asChild>
                        <Link href="/fleet/tachograph-downloads/create"><Plus className="mr-2 size-4" />New</Link>
                    </Button>
                </div>
                ...
                {tachographDownloads.data.length === 0 ? (
                    <div className="rounded-lg border border-dashed py-16 text-center">
                        ...
                        <p className="mt-2 text-sm text-muted-foreground">No tachograph downloads yet.</p>
                        <Button asChild className="mt-4"><Link href="/fleet/tachograph-downloads/create">Add download</Link></Button>
                    </div>
                ) : (
                    <>
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b bg-muted/50">
                                        ...
                                        <th className="p-3 text-left font-medium">Download date</th>
                                        <th className="p-3 text-left font-medium">Status</th>
                                        <th className="p-3 text-left font-medium">Analysed at</th>
                                ...
                                <tbody>
                                    {tachographDownloads.data.map((row) => (
                                        <tr key={row.id} className="border-b last:border-0">
                                            ...
                                            <td className="p-3">{new Date(row.download_date).toLocaleDateString()}</td>
                                            <td className="p-3">{row.status}</td>
                                            <td className="p-3">{row.analysed_at ? new Date(row.analysed_at).toLocaleString() : '—'}</td>
                                            <td className="p-3 text-right">
                                                <Button variant="outline" size="sm" asChild><Link href={`/fleet/tachograph-downloads/${row.id}`}>View</Link></Button>
                                                <Button variant="outline" size="sm" asChild><Link href={`/fleet/tachograph-downloads/${row.id}/edit`}><Pencil className="ml-1 size-3.5" /></Link></Button>
                                                <Form action={`/fleet/tachograph-downloads/${row.id}`} method="delete" className="ml-2 inline" onSubmit={(e) => { if (!confirm('Delete?')) e.preventDefault(); }}>
