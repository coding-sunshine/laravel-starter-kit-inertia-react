...
export default function FleetParkingAllocationsCreate({
    vehicles,
    locations,
}: Props) {
    const form = useForm({
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <div className="flex items-center gap-4">
                    <Button variant="ghost" size="sm" asChild>
                        <Link href="/fleet/parking-allocations">Back</Link>
                    </Button>
                    <h1 className="text-2xl font-semibold">
                        New parking allocation
                    </h1>
                </div>
                <form
                    onSubmit={(e) => {
                        e.preventDefault();
                        form.post('/fleet/parking-allocations');
                    }}
