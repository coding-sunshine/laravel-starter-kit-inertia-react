...
export default function FleetTrainingCoursesCreate({ categories, deliveryMethods }: Props) {
    const form = useForm({
        course_name: '',
        course_code: '',
        description: '',
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: 'New', href: '/fleet/training-courses/create' },
    ];
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Fleet – New training course" />
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <div className="flex items-center gap-4">
                    <Button variant="ghost" size="sm" asChild><Link href="/fleet/training-courses">Back</Link></Button>
                    <h1 className="text-2xl font-semibold">New training course</h1>
                </div>
                <form onSubmit={(e) => { e.preventDefault(); form.post('/fleet/training-courses'); }} className="max-w-xl space-y-4 rounded-lg border p-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <Input type="number" step="0.5" min={0} value={form.data.duration_hours} onChange={e => form.setData('duration_hours', e.target.value)} />
                    </div>
                    <div className="flex gap-2">
                        <Button type="submit" disabled={form.processing}>Save</Button>
                        <Button type="button" variant="outline" asChild><Link href="/fleet/training-courses">Cancel</Link></Button>
                    </div>
                </form>
            </div>
        </AppLayout>
