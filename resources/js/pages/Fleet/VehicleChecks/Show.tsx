...
export default function VehicleChecksShow({ vehicleCheck }: Props) {
    ...
    const driverName = vehicleCheck.performed_by_driver
        ? vehicleCheck.performed_by_driver.first_name +
          ' ' +
          vehicleCheck.performed_by_driver.last_name
        : '—';
    return (
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button variant="ghost" size="sm" asChild>
                            <Link href="/fleet/vehicle-checks">Back</Link>
                        </Button>
                    </div>
                </div>
                <dl className="max-w-md space-y-2 rounded-lg border p-6">
                    ...
                    <div>
                        ...
                        <dd>{vehicleCheck.vehicle?.registration ?? '—'}</dd>
                    </div>
                    <div>
                        <dt className="text-sm text-muted-foreground">
                            Template
                        ...
                        <dd>
                            {vehicleCheck.vehicle_check_template?.name ?? '—'}
                        </dd>
                    </div>
                    <div>
                        <dt className="text-sm text-muted-foreground">
                    ...
                    <div>
                        ...
                        <dd>{vehicleCheck.performed_by_user?.name ?? '—'}</dd>
                    </div>
                    <div>
                        <dt className="text-sm text-muted-foreground">
                            Defect
                        </dt>
                        <dd>{vehicleCheck.defect?.defect_number ?? '—'}</dd>
                    </div>
                </dl>
                {vehicleCheck.vehicle_check_items &&
                    vehicleCheck.vehicle_check_items.length > 0 && (
                        <div>
                            ...
                            <div className="rounded-md border">
                                <table className="w-full text-sm">
                                    ...
                                    <tbody>
                                        {vehicleCheck.vehicle_check_items.map(
                                            (item) => (
                                                <tr
                                                    key={item.id}
