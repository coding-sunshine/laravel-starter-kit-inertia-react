...
export default function FleetTelematicsDevicesEdit({ telematicsDevice, statuses, vehicles }: Props) {
    const installedAt = telematicsDevice.installed_at ? new Date(telematicsDevice.installed_at).toISOString().slice(0, 16) : '';
    const form = useForm({
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: 'Edit', href: `/fleet/telematics-devices/${telematicsDevice.id}/edit` },
    ];
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
                <h1 className="text-2xl font-semibold">Edit telematics device</h1>
                <form onSubmit={handleSubmit} className="max-w-xl space-y-4">
                    ...
                    <div className="flex gap-2">
                        <Button type="submit" disabled={processing}>Update device</Button>
                        <Button variant="outline" asChild>
                            <Link href={`/fleet/telematics-devices/${telematicsDevice.id}`}>Cancel</Link>
                        </Button>
                    </div>
                </form>
            </div>
