...
import { index as rollupsIndex, recalculate as rollupsRecalculate } from '@/routes/daily-vehicle-entry-rollups';
...
interface RollupRow {
    ...
    completed_net_wt_mt: string | number;
    pending_gross_wt_mt: string | number;
...
interface DaySummaryRow {
    ...
    total_completed_net_mt: string;
    total_pending_gross_mt: string;
    has_entry_source: boolean;
}
...
export default function DailyVehicleEntryRollupsIndex({
    days,
    detailRollups,
    filters,
    flash,
    ...
    const breadcrumbs: BreadcrumbItem[] = useMemo(
        () => [
            {
                title: 'Daily vehicle entry rollups',
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Daily vehicle entry rollups" />

            <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 p-6">
                <Heading title="Daily vehicle entry rollups" />
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            Lists every calendar day in the range (newest first). Buckets match{' '}
                            <code className="rounded bg-muted px-1 py-0.5 text-xs">entry_date</code> on{' '}
                            <strong>road dispatch</strong> daily vehicle entries only. Default range is the last 14 days
                            including today.
                        </CardDescription>
                    </CardHeader>
                    <CardContent className="flex flex-col gap-4">
                        ...
                        <div className="flex flex-wrap items-end gap-4">
                            <div className="flex flex-col gap-2">
                                <Label htmlFor="dve-rollup-from">From</Label>
                                <Input
                                    id="dve-rollup-from"
                                    type="date"
                                    value={rangeFrom}
                            ...
                            <div className="flex flex-col gap-2">
                                <Label htmlFor="dve-rollup-to">To</Label>
                                <Input
                                    id="dve-rollup-to"
                                    type="date"
                                    value={rangeTo}
                            ...
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                        ...
                                        <TableHead className="text-right whitespace-nowrap">Total entries</TableHead>
                                        <TableHead className="text-right whitespace-nowrap">Σ net MT (completed)</TableHead>
                                        <TableHead className="text-right whitespace-nowrap">Σ gross MT (pending)</TableHead>
                                    ...
                                    {days.data.length === 0 ? (
                                        ...
                                        days.data.map((row) => (
                                            <TableRow key={row.date}>
                                                ...
                                                <TableCell className="text-right">{row.total_completed_net_mt}</TableCell>
                                                <TableCell className="text-right">{row.total_pending_gross_mt}</TableCell>
                                                <TableCell className="text-muted-foreground text-sm">
                                                    {row.has_entry_source ? (
                                                        <span className="text-foreground">Has road dispatch rows</span>
                                                    ) : (
                                                        <span>No road dispatch rows</span>
                                                    )}
                                                </TableCell>
                                                <TableCell className="text-right">
                                                    <div className="flex flex-wrap justify-end gap-2">
                                                        ...
                                                        <Button
                                                            ...
                                                            size="sm"
                                                            disabled={
                                                                !row.has_entry_source || submittingDay === row.date
                                                            }
                                                            title={
                                                                row.has_entry_source
                                                                    ? undefined
                                                                    : 'Create road dispatch entries for this date first.'
                                                            }
                    ...
                    <Card>
                        <CardHeader className="flex flex-row flex-wrap items-start justify-between gap-4">
                            <div>
                                ...
                                <CardDescription>
                                    Per siding / shift rollup rows ({detailRollups?.total ?? 0} row(s)).
                                </CardDescription>
                            </div>
                                ...
                                <Table>
                                    <TableHeader>
                                        <TableRow>
                                            ...
                                            <TableHead className="whitespace-nowrap">rollup_day</TableHead>
                                            ...
                                            <TableHead className="text-right whitespace-nowrap">entries_count</TableHead>
                                            ...
                                            <TableHead className="text-right whitespace-nowrap">net MT</TableHead>
                                            <TableHead className="text-right whitespace-nowrap">pending gross MT</TableHead>
                                        ...
                                        {!detailRollups || detailRollups.data.length === 0 ? (
                                            <TableRow>
                                                <TableCell colSpan={11} className="text-muted-foreground">
                                                    No rollup buckets for this date yet — recalculate after entries exist.
                                                </TableCell>
                                            </TableRow>
                                        ) : (
                                            detailRollups.data.map((r) => (
                                                <TableRow key={r.id}>
                                                    ...
                                                    <TableCell className="whitespace-nowrap">{r.rollup_day}</TableCell>
                                                    <TableCell>{r.siding_id}</TableCell>
                                                    <TableCell>
                                                        {r.siding ? `${r.siding.name} (${r.siding.code})` : '—'}
                                                    </TableCell>
                                                    <TableCell>{r.shift}</TableCell>
                                                    <TableCell className="text-right">{r.entries_count}</TableCell>
                                                    <TableCell className="text-right">{r.completed_entries_count}</TableCell>
                                                    <TableCell className="text-right">{r.pending_entries_count}</TableCell>
                                                    <TableCell className="text-right">{r.completed_net_wt_mt}</TableCell>
                                                    <TableCell className="text-right">{r.pending_gross_wt_mt}</TableCell>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            Use when operators logged vehicles under a sheet date that is not in your current range list.
                            Rebuild runs from{' '}
                            <code className="rounded bg-muted px-1 py-0.5 text-xs">daily_vehicle_entries</code>{' '}
                            (<strong>road_dispatch</strong> only). If no rows exist for that date, validation fails.
                        </CardDescription>
                    </CardHeader>
                    <CardContent className="flex flex-wrap items-end gap-4">
                        <div className="flex flex-col gap-2">
                            <Label htmlFor="dve-rollup-orphan-date">Date</Label>
                            <Input
                                id="dve-rollup-orphan-date"
                                type="date"
                                value={orphanDate}
