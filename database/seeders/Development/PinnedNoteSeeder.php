...
final class PinnedNoteSeeder extends Seeder
{
    ...
    {
        try {
            $data = $this->loadJson('pinned_notes.json');

            if (! isset($data['pinned_notes']) || ! is_array($data['pinned_notes'])) {
                return;
            }

            foreach ($data['pinned_notes'] as $itemData) {
                $factoryState = $itemData['_factory_state'] ?? null;
                unset($itemData['_factory_state']);

                // Use idempotent updateOrCreate if unique field exists
                if (false) {
                    PinnedNote::query()->updateOrCreate(
                        ['id' => $itemData['id']],
                        $itemData
                    );
                } else {
                    // Fallback to factory if no unique field
                    $factory = PinnedNote::factory();
    ...
    private function seedFromFactory(): void
    {
        ...
        if (method_exists(PinnedNote::factory(), 'admin')) {
            PinnedNote::factory()
                ->admin()
                ->count(2)
                ->create();
