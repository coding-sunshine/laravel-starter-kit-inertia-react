...
`app/Actions/ProcessGuardInspection.php`
...
```php
app(ProcessGuardInspection::class)->handle($params);
```

### From Job/Command

```php
(new ProcessGuardInspection($dependency))->handle($params);
```
