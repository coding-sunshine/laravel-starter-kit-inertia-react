# SyncTxrUnfitFlagsToWagonsAction
...
`app/Actions/SyncTxrUnfitFlagsToWagonsAction.php`
...
```php
app(SyncTxrUnfitFlagsToWagonsAction::class)->handle($params);
```

### From Job/Command

```php
(new SyncTxrUnfitFlagsToWagonsAction($dependency))->handle($params);
```
