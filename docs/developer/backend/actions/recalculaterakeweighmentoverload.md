# RecalculateRakeWeighmentOverload
...
`app/Actions/RecalculateRakeWeighmentOverload.php`
...
```php
app(RecalculateRakeWeighmentOverload::class)->handle($params);
```

### From Job/Command

```php
(new RecalculateRakeWeighmentOverload($dependency))->handle($params);
```
