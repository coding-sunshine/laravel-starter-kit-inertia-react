# GenerateLoadingRecommendationAction
...
`app/Actions/GenerateLoadingRecommendationAction.php`
...
```php
app(GenerateLoadingRecommendationAction::class)->handle($params);
```

### From Job/Command

```php
(new GenerateLoadingRecommendationAction($dependency))->handle($params);
```
