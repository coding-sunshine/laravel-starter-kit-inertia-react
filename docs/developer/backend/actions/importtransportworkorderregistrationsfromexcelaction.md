# ImportTransportWorkOrderRegistrationsFromExcelAction
...
`app/Actions/ImportTransportWorkOrderRegistrationsFromExcelAction.php`
...
```php
app(ImportTransportWorkOrderRegistrationsFromExcelAction::class)->handle($params);
```

### From Job/Command

```php
(new ImportTransportWorkOrderRegistrationsFromExcelAction($dependency))->handle($params);
```
