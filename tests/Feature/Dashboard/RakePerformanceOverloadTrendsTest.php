...
test('overload trends returns siding-wise daily percentages for today', function (): void {
    ...
    $response->assertOk()
        ...
        ->assertJsonPath('data.underload_threshold', 1)
    ...
    expect($sidingRow['siding_id'])->toBe($siding->id)
        ...
        ->and($sidingRow['summary']['avg_daily_overload_pct'])->toBe(33.3)
        ->and($sidingRow['summary']['avg_daily_underload_pct'])->toBe(33.3);

    $todayPoint = collect($sidingRow['daily'])->first(
        fn (array $d): bool => $d['total_wagons'] > 0,
    );
    expect($todayPoint)->not->toBeNull()
        ->and($todayPoint['overload_pct'])->toBe(33.3)
        ->and($todayPoint['underload_pct'])->toBe(33.3);
});
