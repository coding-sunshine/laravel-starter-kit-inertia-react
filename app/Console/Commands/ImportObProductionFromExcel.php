...
final class ImportObProductionFromExcel extends Command
{
    protected $signature = 'production:import-ob
                            {--file= : Path to xlsx (default: database/excel/ob production.xlsx)}
                            {--sheet= : Sheet name (default: first sheet)}
                            {--dry-run : Parse only, do not write to DB}
                            {--log-skipped : Print rows skipped during import}';

    protected $description = 'Import month-wise OB production from Excel into production_entries';

    public function handle(ProductionExcelImporter $importer): int
    {
        $resolvedPath = $this->resolvePath(
            $this->option('file'),
            database_path('excel/ob production.xlsx')
        );
        ...
        try {
            $stats = $importer->import($resolvedPath, ProductionEntry::TYPE_OB, $sheetName, $dryRun, $logSkipped);
        } catch (Throwable $e) {
        ...
        $this->info('OB production import completed.');
        $this->table(
