...
final class UpdateVehicleWorkorderRequest extends FormRequest
{
    public function authorize(): bool
    {
        ...
        return $this->user()?->canAccessSiding($workorder->siding_id) ?? false;
    }
    ...
    {
        $nullable = [
            'vehicle_no', 'rcd_pin_no', 'transport_name', 'wo_no', 'wo_no_2',
            'work_order_date', 'issued_date', 'proprietor_name', 'represented_by',
            'place', 'address', 'tyres', 'tare_weight', 'mobile_no_1', 'mobile_no_2',
            'owner_type', 'regd_date', 'permit_validity_date', 'tax_validity_date',
            'fitness_validity_date', 'insurance_validity_date', 'maker_model',
            'make', 'model', 'remarks', 'recommended_by', 'referenced',
            'local_or_non_local', 'pan_no', 'gst_no',
        ];
        $data = $this->all();
