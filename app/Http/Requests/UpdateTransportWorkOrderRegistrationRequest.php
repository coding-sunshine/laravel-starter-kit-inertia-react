...
final class UpdateTransportWorkOrderRegistrationRequest extends FormRequest
{
    ...
    {
        $registration = $this->route('tw_registration');
        $id = $registration instanceof TransportWorkOrderRegistration ? $registration->id : 0;

        return [
            'siding_id' => ['required', 'integer', 'exists:sidings,id'],
            'work_order_no_1' => $this->workOrderNoRules(),
            'work_order_no_2' => [...$this->workOrderNoRules(), Rule::unique('transport_work_order_registrations', 'work_order_no_2')->ignore($id)],
    ...
    public function withValidator(Validator $validator): void
    {
        ...
        $validator->after(function (Validator $validator) use ($registration): void {
            if ($registration instanceof TransportWorkOrderRegistration) {
                $wo1 = $this->filled('work_order_no_1')
                    ? $this->normalizeWorkOrderValue($this->input('work_order_no_1'))
                    : $registration->work_order_no_1;
                $wo2 = $this->filled('work_order_no_2')
                    ? $this->normalizeWorkOrderValue($this->input('work_order_no_2'))
                    : $registration->work_order_no_2;
