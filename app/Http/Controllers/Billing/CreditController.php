...
final readonly class CreditController
{
    ...
    public function checkoutLemonSqueezy(Request $request): RedirectResponse
    {
        ...
        $model = \App\Models\Billing\PaymentGateway::query()
            ->where('type', 'lemon_squeezy')
            ->where('is_active', true)
            ->first();
        if ($model instanceof \App\Models\Billing\PaymentGateway && $gateway instanceof \App\Services\PaymentGateway\Gateways\LemonSqueezyGateway) {
