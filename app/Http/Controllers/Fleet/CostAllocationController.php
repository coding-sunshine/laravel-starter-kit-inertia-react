...
final class CostAllocationController extends Controller
{
    public function index(Request $request): Response
    {
        ...
        return Inertia::render('Fleet/CostAllocations/Index', [
            ...
            'costTypes' => array_map(fn ($c) => ['value' => $c->value, 'name' => $c->name], CostAllocationType::cases()),
            'sourceTypes' => array_map(fn ($c) => ['value' => $c->value, 'name' => $c->name], CostAllocationSourceType::cases()),
            'approvalStatuses' => array_map(fn ($c) => ['value' => $c->value, 'name' => $c->name], CostAllocationApprovalStatus::cases()),
        ]);
    }
    ...
    {
        $this->authorize('create', CostAllocation::class);
        CostAllocation::create(array_merge($request->validated(), ['allocated_by' => $request->user()->id]));
