...
final class MileageClaimController extends Controller
{
    public function index(): Response
    {
        ...
        $claims = MileageClaim::query()->with(['greyFleetVehicle', 'user'])->latest('claim_date')->paginate(15)->withQueryString();

        return Inertia::render('Fleet/MileageClaims/Index', [
            'mileageClaims' => $claims,
            'greyFleetVehicles' => GreyFleetVehicle::query()->orderBy('registration')->get(['id', 'registration', 'make', 'model'])->map(fn ($v): array => ['id' => $v->id, 'label' => $v->registration.' – '.mb_trim(($v->make ?? '').' '.($v->model ?? ''))]),
            'users' => User::query()->orderBy('name')->get(['id', 'name']),
            'statuses' => array_map(fn (MileageClaimStatus $c): array => ['value' => $c->value, 'name' => $c->name], MileageClaimStatus::cases()),
        ]);
    }
    ...
    {
        $this->authorize('create', MileageClaim::class);
        MileageClaim::query()->create($request->validated());
