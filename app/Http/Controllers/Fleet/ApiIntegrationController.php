...
final class ApiIntegrationController extends Controller
{
    public function index(): Response
    {
        ...
        return Inertia::render('Fleet/ApiIntegrations/Index', [
            ...
            'integrationTypes' => array_map(fn (ApiIntegrationType $c): array => ['value' => $c->value, 'name' => $c->name], ApiIntegrationType::cases()),
            'syncStatuses' => array_map(fn (ApiIntegrationSyncStatus $c): array => ['value' => $c->value, 'name' => $c->name], ApiIntegrationSyncStatus::cases()),
        ]);
    }
    ...
    {
        $this->authorize('create', ApiIntegration::class);
        ApiIntegration::query()->create($request->validated());
