...
final class ContractorComplianceController extends Controller
{
    public function index(): Response
    {
        ...
        $items = ContractorCompliance::query()->with('contractor')->orderByDesc('expiry_date')->paginate(15)->withQueryString();
        ...
        return Inertia::render('Fleet/ContractorCompliance/Index', [
            ...
            'complianceTypes' => array_map(fn ($c) => ['value' => $c->value, 'name' => $c->name], ContractorComplianceType::cases()),
            'statuses' => array_map(fn ($c) => ['value' => $c->value, 'name' => $c->name], ContractorComplianceStatus::cases()),
        ]);
    }
    ...
    {
        $this->authorize('create', ContractorCompliance::class);
        ContractorCompliance::create($request->validated());
