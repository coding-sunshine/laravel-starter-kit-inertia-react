...
final class TrainingEnrollmentController extends Controller
{
    public function index(Request $request): Response
    {
        ...
        return Inertia::render('Fleet/TrainingEnrollments/Index', [
            ...
            'trainingSessions' => TrainingSession::query()->with('trainingCourse')->orderByDesc('scheduled_date')->get(['id', 'session_name', 'scheduled_date', 'training_course_id'])->map(fn ($s) => ['id' => $s->id, 'name' => $s->session_name . ' (' . $s->scheduled_date->format('Y-m-d') . ')']),
            'drivers' => Driver::query()->orderBy('last_name')->get(['id', 'first_name', 'last_name'])->map(fn ($d) => ['id' => $d->id, 'name' => $d->first_name . ' ' . $d->last_name]),
            'enrollmentStatuses' => array_map(fn ($c) => ['value' => $c->value, 'name' => $c->name], TrainingEnrollmentStatus::cases()),
            'passFailOptions' => array_map(fn ($c) => ['value' => $c->value, 'name' => $c->name], TrainingPassFail::cases()),
        ]);
    }
    ...
    public function create(): Response
    {
        ...
        return Inertia::render('Fleet/TrainingEnrollments/Create', [
            'trainingSessions' => TrainingSession::query()->with('trainingCourse')->orderByDesc('scheduled_date')->get(['id', 'session_name', 'scheduled_date'])->map(fn ($s) => ['id' => $s->id, 'name' => $s->session_name . ' (' . $s->scheduled_date->format('Y-m-d') . ')']),
            'drivers' => Driver::query()->orderBy('last_name')->get(['id', 'first_name', 'last_name'])->map(fn ($d) => ['id' => $d->id, 'name' => $d->first_name . ' ' . $d->last_name]),
            'enrollmentStatuses' => array_map(fn ($c) => ['value' => $c->value, 'name' => $c->name], TrainingEnrollmentStatus::cases()),
            'passFailOptions' => array_map(fn ($c) => ['value' => $c->value, 'name' => $c->name], TrainingPassFail::cases()),
        ]);
    ...
    {
        $this->authorize('create', TrainingEnrollment::class);
        TrainingEnrollment::create(array_merge($request->validated(), ['enrolled_by' => $request->user()->id]));
