...
final class DashcamClipController extends Controller
{
    public function index(Request $request): Response
    {
        ...
        return Inertia::render('Fleet/DashcamClips/Index', [
            ...
            'incidents' => Incident::query()->orderByDesc('incident_date')->limit(200)->get(['id', 'incident_number']),
        ]);
    }
    ...
    public function create(): Response
    {
        ...
        return Inertia::render('Fleet/DashcamClips/Create', [
            ...
            'drivers' => Driver::query()->orderBy('last_name')->get(['id', 'first_name', 'last_name'])->map(fn ($d) => ['id' => $d->id, 'name' => $d->first_name . ' ' . $d->last_name]),
            'incidents' => Incident::query()->orderByDesc('incident_date')->limit(200)->get(['id', 'incident_number']),
            'eventTypes' => array_map(fn ($c) => ['value' => $c->value, 'name' => $c->name], DashcamClipEventType::cases()),
            'statuses' => array_map(fn ($c) => ['value' => $c->value, 'name' => $c->name], DashcamClipStatus::cases()),
        ]);
    }
    ...
    {
        $this->authorize('create', DashcamClip::class);
        DashcamClip::create($request->validated());
